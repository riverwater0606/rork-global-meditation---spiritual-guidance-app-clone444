...
function buildSnapshotPath(walletAddress) {
  return `vip-entitlement/v1/${sanitizeSegment(walletAddress)}.json`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch VIP entitlement (${response.status})`);
      }
    ...
    if (req.method === "DELETE") {
      ...
      return json(res, 200, { ok: true, deleted: true, serverNow: new Date().toISOString() });
    }

    return json(res, 405, { error: "Method not allowed" });
  } catch (error) {
    return json(res, 500, {
      error: error instanceof Error ? error.message : "VIP entitlement sync failed",
      serverNow: new Date().toISOString(),
    });
  }
};
