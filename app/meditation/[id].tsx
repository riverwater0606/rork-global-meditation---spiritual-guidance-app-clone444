...
export default function MeditationPlayerScreen() {
  ...
  const [durationSeconds, setDurationSeconds] = useState<number>(session?.duration ? session.duration * 60 : 0);
  const [timeRemaining, setTimeRemaining] = useState<number>(session?.duration ? session.duration * 60 : 0);
  ...
  useEffect(() => {
    if (session?.duration) {
      const defaultSeconds = session.duration * 60;
  ...
  const onPlaybackStatusUpdate = useCallback(
    ...
    [completeMeditation, durationSeconds, resetSpeech, session]
  );
      ...
      <SafeAreaView style={styles.safeArea}>
        <Animated.View style={[styles.content, { opacity: fadeAnimation }]}>
          ...
          <View style={styles.controls}>
            ...
            <AnimatedTouchable style={styles.playButton} onPress={togglePlayback} testID="play-pause-button">
              {isPlaying ? (
