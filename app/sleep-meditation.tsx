...
import { Clock, Moon, Play, Sparkles, Star } from "lucide-react-native";
...
const TRANSLATIONS: Record<SupportedLanguage, {
  ...
  en: {
    ...
    subtitle: "Drift into peaceful slumber",
    recommended: "Recommended for Sleep",
    minutes: "min",
    badge: "AI Guided",
  },
  zh: {
...
export default function SleepMeditationScreen() {
  ...
  const catalogSessions = MEDITATION_SESSIONS.filter((session) => session.category === "sleep");
  const personalizedSessions = customSessions.filter((session) => session.category === "sleep");
  const sleepSessions = [...personalizedSessions, ...catalogSessions];
          ...
          {sleepSessions.map((session) => {
            ...
            return (
              <TouchableOpacity
                ...
                testID={`sleep-session-${session.id}`}
              >
                <LinearGradient
          ...
          {sleepSessions.length === 0 && (
            <View style={[styles.emptyState, { borderColor: currentTheme.border }]}>
              <Text style={[styles.emptyText, { color: currentTheme.textSecondary }]}>
                {lang === "zh" ? "目前沒有睡眠冥想課程" : "No sleep meditations yet"}
              </Text>
            </View>
          )}
        </View>
