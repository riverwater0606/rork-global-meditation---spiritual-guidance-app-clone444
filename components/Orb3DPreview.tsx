...
const OrbParticlesPreview = ({ layers, size, shape }: { layers: string[]; size: number; shape: OrbShape }) => {
  ...
  const { positions, colors } = useMemo(() => {
    ...
    if (shape === 'merkaba') {
      ...
      const intersectionCount = Math.floor(particleCount * 0.15);
      ...
      for(let i=0; i<circleCount; i++) {
          ...
          positions[pIdx*3] = center.x + circleRadius * Math.cos(theta);
          positions[pIdx*3+1] = center.y + circleRadius * Math.sin(theta);
          positions[pIdx*3+2] = (Math.random() - 0.5) * 0.02;
      ...
      for(let i=0; i<intersectionCount; i++) {
          ...
          positions[pIdx*3] = point.x + (Math.random()-0.5)*spread;
          positions[pIdx*3+1] = point.y + (Math.random()-0.5)*spread;
          positions[pIdx*3+2] = (Math.random()-0.5)*spread + 0.08;
