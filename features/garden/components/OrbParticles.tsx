...
export const OrbParticles = ({ layers, interactionState, shape }: { layers: string[], interactionState: any, shape: OrbShape }) => {
  ...
  const jitterPhase = useMemo(() => {
    const phase = new Float32Array(PARTICLE_COUNT * 3);
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (mode === 'gather') {
        ...
        tx += Math.sin(t * 7 + jitterPhase[ix]) * jitter;
        ty += Math.sin(t * 7 + jitterPhase[iy]) * jitter;
        tz += Math.sin(t * 7 + jitterPhase[iz]) * jitter;
      } 
      else if (mode === 'heart') {
        ...
        tz *= 0.01;
         ...
         const flyScale = 2.0;
         tx *= flyScale + Math.sin(t * 9 + jitterPhase[ix]) * 0.25;
         ty = ty * flyScale + 5.0; // Fly UP off screen
         tz *= flyScale + Math.sin(t * 9 + jitterPhase[iz]) * 0.25;
      }
